...
| `--brass-deep` | `#886830` | Solid fills — the primary filled button (`.btn--filled`). `--brass` reads as flat yellow at full-block scale; this darker, richer gold is reserved for large solid fills specifically. Hovers up to `--brass`. |
| `--bronze` | `#a87b4a` | Warmer metal — the cone tables, lamp bases. Secondary metallic accent. |

> **Updated Jul 2026:** the brass hue was shifted a few degrees warmer (was `#c4a440`/`#dcc06a`/`#8a6e28`) — the old values read as flat yellow rather than a metal. Same lightness/saturation, just less yellow, more gold-orange. Applies everywhere `--brass*` is used, including the hardcoded rgba() glows/tints in `globals.css`, `pages.css`, and `opening-sequence.css` that echo these hex values. `--brass-deep` was added afterward once the client saw a full-block button in the (still lighter) `--brass` and asked for a noticeably darker gold — small text/hairline uses of `--brass` read fine; a whole button filled with it read yellow.
...
- **Legibility: solid "plaque" per line, not an overlay or an outline.** This went through three iterations before landing:
  1. A soft ivory "spotlight" scrim behind the whole text block — read as a haze sitting over the artwork.
  2. Dark ink text (chosen from sampling the mural's actual pixels — the wallpaper is a muted mid-dark olive, not the light pastel it looks like in thumbnail) with a white outline/glow for edge definition — on a real iPhone this needed to be thick to survive crossing the peacock's feathers, and at that thickness it read as an ugly white background around the letters, not a subtle rim.
  3. **Current approach:** each line of `.hero-title` and `.hero-sub` sits on its own tightly-fitted solid dark plaque (`rgba(10,8,7,.78)`, small padding, 3px radius), using `box-decoration-break: clone` on a `<span class="line-bg">` wrapping the text so a *forced* line break (`.hero-title`'s `<br>`) or a *natural* wrap (`.hero-sub`) each get their own independently-sized background box — not one big rectangle spanning the widest line. Text goes back to the same `--ivory` / `--ivory-dark` used everywhere else on the site, since the plaque, not the mural, is now what it contrasts against. No outline, no glow, no dependency on what's behind it.
  - This is the same device already used for `.hero-label` ("Coming Soon" — a small dark chip with full-strength `--brass-light` text), just extended to the rest of the hero copy for one consistent "museum placard" language, rather than mixing a badge with an outline hack.
  - Side effect: this also resolved an apparent "title isn't centered" report — the text was always centered (verified via `getBoundingClientRect`, dead center at every breakpoint tested), but the uneven white outline made it look off-axis. A rectangular plaque makes true centering visually unambiguous.
- **All hero/nav chips are solid, not glass:** `.hero-label`, `.hero-ctas .btn--ghost`, the pre-scroll `.nav-reserve`, `#nav-links` items, and `.nav-toggle` all use a flat `rgba(10,8,7,.75–.85)` fill, no `backdrop-filter`. An earlier pass used `backdrop-filter: blur()` over a translucent fill, which on a real iPhone rendered as a smeared, dirty-looking patch wherever the chip sat over a high-contrast part of the mural (e.g. a magnolia petal edge) — a known Safari risk. A solid chip can't smear on any device. The reusable global `.btn--ghost` is untouched for use elsewhere; only the hero's instance is overridden.
- **Nav, pre-scroll only:** the shared header sits transparently over the hero until `.is-scrolled` is added on scroll. `home.css` (loaded only on `index.html`) gives `#nav-links li`, `.nav-reserve`, and `.nav-toggle` a small solid dark plaque under `#site-header:not(.is-scrolled)` — the text/icon colors are untouched, they're the same ones the site already uses on dark backgrounds everywhere else. Once scrolled, the header's own dark backdrop makes the chips redundant and the standard nav styling applies as normal.
