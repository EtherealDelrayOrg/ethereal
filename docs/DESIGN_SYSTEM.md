...
>
> `.btn--filled`'s color has moved a few times chasing "more gold, less yellow" then "too dark" then "more brass gold": a `--brass-deep` (darker) variant was tried and reverted for reading too dark, then a plain `--brass` fill read a little flat/pale at full-block scale. It now uses its own richer, more saturated brass gold (`#d48e2b`, hover `#daa458`) with a matching-hue darker border (`#936525`) — deliberately not the shared `--brass` token, since what reads right for a small hairline/text accent and what reads right as a large solid fill are different jobs. Kept the faint inset top/bottom bevel (`box-shadow: inset 0 1px 0 rgba(255,255,255,.25), inset 0 -1px 0 rgba(0,0,0,.2)`) from the earlier round — the contrasting border + bevel reads as a struck metal plaque (like the clock bezel), not a flat colored rectangle. `--brass-deep` was removed as a token; nothing uses it. `--brass` itself is untouched for nav/hairline/text use.
