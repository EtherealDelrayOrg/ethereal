...
1. **The image keeps getting brighter on request.** `.hero-bg`: `filter: brightness(1.15) saturate(1.14)`, up from `1.05`, up from an earlier dip to `0.88` for text contrast. Each time the client has asked for more brightness, the scrim below has been strengthened to compensate instead of dimming the photo back down — the direction of travel is "the image wins, the scrim adapts," not the other way around.
