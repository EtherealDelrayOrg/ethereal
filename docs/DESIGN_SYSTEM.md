...
1. **The image now runs at its native grade — `filter: none` (Jul 2026).** The mural source was replaced with a new higher-res version (1254×1254, same composition) and the old `brightness(1.4) saturate(1.18)` boost — which had climbed on request through `0.88` → `1.05` → `1.15` → `1.3` → `1.4` against the *previous* image — was bleaching the new source's richer teal-grey tone, so it was removed entirely rather than re-tuned (client preference: as close to the original file as possible). **Lesson from the swap:** the filter and the scrim were calibrated as a pair against the old image; changing either the source or the filter means re-checking both, not just one.
2. **A dark (never light) graduated scrim**, `.hero-scrim`, layered as its own element above `.hero-bg` — a radial darken centered on the text block plus a bottom-weighted linear gradient. Eased down twice more after the filter removal (the old strength read proportionally heavier over the dimmer native-grade base): bottom max `.52` → `.38` → **`.24`**, radial center `.30` → `.22` → **`.14`** (top linear now `.03`→`.13`→`.2`→`.24`; the top-weighted header band below is unchanged). Almost nothing across the birds' heads at the top (that's deliberately where the gradient is weakest); heaviest low, where the subtitle and buttons sit. This reads as a photographic grade (like burning in a print), not a haze, because it darkens rather than lightens and has no hard edge.
