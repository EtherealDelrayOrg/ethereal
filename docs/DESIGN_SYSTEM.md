...
1. **The image keeps getting brighter on request.** `.hero-bg`: `filter: brightness(1.3) saturate(1.16)`, up from `1.15`, up from `1.05`, up from an earlier dip to `0.88` for text contrast. Each time the client has asked for more brightness, the scrim below has been strengthened to compensate instead of dimming the photo back down — the direction of travel is "the image wins, the scrim adapts," not the other way around.
2. **A dark (never light) graduated scrim**, `.hero-scrim`, layered as its own element above `.hero-bg` — a radial darken centered on the text block plus a bottom-weighted linear gradient (`rgba(10,8,7,.07)` at the top down to `rgba(10,8,7,.72)` at the bottom). Almost nothing across the birds' heads at the top (that's deliberately where the gradient is weakest); heaviest low, where the subtitle and buttons sit. This reads as a photographic grade (like burning in a print), not a haze, because it darkens rather than lightens and has no hard edge.
