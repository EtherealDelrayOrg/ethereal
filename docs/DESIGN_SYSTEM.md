...
1. **The image itself carries a small grade.** `.hero-bg`: `filter: brightness(0.88) saturate(1.1)` (was `1.05`) — still clearly a bright, vivid photograph, just enough depth for text to land on.
2. **A dark (never light) graduated scrim**, `.hero-scrim`, layered as its own element above `.hero-bg` — a radial darken centered on the text block plus a bottom-weighted linear gradient (`rgba(10,8,7,.05)` at the top down to `rgba(10,8,7,.66)` at the bottom). Almost nothing across the birds' heads at the top (that's deliberately where the gradient is weakest); heaviest low, where the subtitle and buttons sit. This reads as a photographic grade (like burning in a print), not a haze, because it darkens rather than lightens and has no hard edge.
